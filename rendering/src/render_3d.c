...
void render_3d_view(t_game *game)
{
    ...
    data.projection_distance = (WINDOW_WIDTH / 2.0) / tan(data.fov_radians / 2.0);
    data.angle_step = data.fov_radians / data.ray_count;
    ray_index = 0;
    while (ray_index < data.ray_count)
    {
        ray_angle = game->player.rotation - (data.fov_radians / 2.0) + (ray_index * data.angle_step);
        draw_wall_strip(game, ray_angle, ray_index * data.strip_width, data.strip_width, data.projection_distance);
        ray_index++;
    }
}
...
void draw_wall_strip(t_game *game, double ray_angle, int screen_x, int strip_width, double projection_distance)
{
    ...
    draw_textured_wall_strip(game, &ray, screen_x, strip_width, wall_top, wall_bottom);
}
