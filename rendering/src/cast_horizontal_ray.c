...
{
    data->y_intercept = floor((game->player.y * data->tile_size) / data->tile_size) * data->tile_size;
    if (sin(ray->angle) > 0)
        data->y_intercept += data->tile_size;
    data->x_intercept = (game->player.x * data->tile_size) + 
        (data->y_intercept - (game->player.y * data->tile_size)) / tan(ray->angle);
    if (sin(ray->angle) > 0)
