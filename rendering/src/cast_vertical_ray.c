...
{
    data->x_intercept = floor((game->player.x * data->tile_size) / data->tile_size) * data->tile_size;
    if (cos(ray->angle) > 0)
        data->x_intercept += data->tile_size;
    data->y_intercept = (game->player.y * data->tile_size) + 
        (data->x_intercept - (game->player.x * data->tile_size)) * tan(ray->angle);
    if (cos(ray->angle) > 0)
